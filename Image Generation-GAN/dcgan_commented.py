...
import torchvision.utils as vutils #torch vision to generate the images  
...
def weights_init(m):
    ...
    if classname.find('Conv') != -1:
        m.weight.data.normal_(0.0, 0.02)# normal dist centred at 0 std 0.02
    elif classname.find('BatchNorm') != -1:
        m.weight.data.normal_(1.0, 0.02)# normal dist centred at 1 std 0.02, It preserves the quality of Batchnorm, which can recognize features not in train set. 
        m.bias.data.fill_(0)#Bias at batchnorm level is 0
...
for epoch in range(25): # We iterate over 25 epochs.
    ...
    for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
        # whole dataset broken into minibatches of data
        ...
        # input.size()[0] is the batch size.
        ...
        # detach() to remove the gradients from the 'fake' tensor
        ...
        # detach() not used as we would require the gradient to update the weights
